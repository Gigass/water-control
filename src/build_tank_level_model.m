...
% Feedforward path
add_block('simulink/User-Defined Functions/Fcn', blk('InvSqrt'), ...
    'Expr', '1/sqrt(u+1e-4)', ...
    'Position', [120 100 170 130]);

ff_gain_value = ctrl.ff_gain * params.q_in_base / params.kv;
add_block('simulink/Sources/Constant', blk('FFGain'), ...
    'Value', num2str(ff_gain_value), ...
...
% Controller (PID block configured as PI)
...
add_block('simulink/Math Operations/Sum', blk('ControlSum'), ...
    'Inputs', '++', ...
    'Position', [250 40 270 90]);
...
add_block('simulink/Sources/Random Number', blk('QinNoise'), ...
    'Mean', '0', ...
    'Variance', '1', ...
    'Seed', '1', ...
    'SampleTime', '0.1', ...
    'Position', [60 430 90 460]);

add_block('simulink/Math Operations/Gain', blk('QinNoiseGain'), ...
    'Gain', num2str(params.q_in_noise_std), ...
    'Position', [110 430 140 460]);
...
add_block('simulink/Continuous/Transfer Fcn', blk('LevelFilter'), ...
    'Numerator', '[1]', ...
    'Denominator', sprintf('[%g 1]', params.filter_tau), ...
    'Position', [380 250 420 290]);
...
add_block('simulink/Continuous/Transfer Fcn', blk('ActuatorLag'), ...
    'Numerator', '[1]', ...
...
add_block('simulink/Signal Routing/Mux', blk('FlowMux'), ...
    'Inputs', '2', ...
    'Position', [520 400 540 460]);

add_block('simulink/Sinks/Scope', blk('FlowScope'), ...
    'NumInputPorts', '1', ...
    'Position', [560 400 590 460]);

%% Connections
add_line(model, 'Setpoint/1', 'ErrorSum/1');
add_line(model, 'Setpoint/1', 'InvSqrt/1', 'autorouting', 'on');
...
add_line(model, 'PI_Controller/1', 'ControlSum/1');
add_line(model, 'ControlSum/1', 'ValveSaturation/1');
add_line(model, 'ValveSaturation/1', 'ActuatorLag/1');
add_line(model, 'ActuatorLag/1', 'DeadbandMap/1');
add_line(model, 'DeadbandMap/1', 'KvGain/1');
...
add_line(model, 'QinNoise/1', 'QinNoiseGain/1');
add_line(model, 'QinNoiseGain/1', 'QinSum/4');
add_line(model, 'QinSum/1', 'FlowSum/1');
add_line(model, 'FlowSum/1', 'AreaGain/1');
add_line(model, 'AreaGain/1', 'LevelIntegrator/1');
add_line(model, 'LevelIntegrator/1', 'LevelFilter/1');
add_line(model, 'LevelFilter/1', 'SqrtLevel/1');
add_line(model, 'LevelFilter/1', 'MeasurementSum/1', 'autorouting', 'on');
...
add_line(model, 'ControlSum/1', 'ValveMux/1', 'autorouting', 'on');
add_line(model, 'ActuatorLag/1', 'ValveMux/2', 'autorouting', 'on');
add_line(model, 'DeadbandMap/1', 'ValveMux/3', 'autorouting', 'on');
add_line(model, 'ValveMux/1', 'ValveScope/1');
add_line(model, 'QinSum/1', 'FlowMux/1', 'autorouting', 'on');
add_line(model, 'QinNoiseGain/1', 'FlowMux/2', 'autorouting', 'on');
add_line(model, 'FlowMux/1', 'FlowScope/1');
